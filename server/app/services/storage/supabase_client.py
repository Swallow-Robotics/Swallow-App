...
class SupabaseClient:
    ...
    def _execute_with_retry(
        ...
        """Run a Supabase execute call, retrying on transient connection errors."""
        last_exc = None
        for attempt in range(max_attempts):
            try:
                ...
                delay = 0.5 * (attempt + 1)
                logging.warning(
                    "Supabase request failed (attempt %s/%s): %s; retrying in %ss",
                    attempt + 1,
                    max_attempts,
                    type(e).__name__,
                    delay,
    ...
    def get_project_location(self, project_id: str) -> Optional[Dict[str, Any]]:
        ...
        response = self._execute_with_retry(_fetch)
        return response.data[0] if response.data else None
