...
| **Yarn** | 1.22+ | Latest | Frontend package manager (`npm install -g yarn`) |
...
supabase>=2.0.0           # Supabase client (metadata, auth)
boto3>=1.34.0             # AWS/R2 client (file storage)
...
# Supabase Configuration (all data/metadata goes through Supabase; no local DB)
...
```
(No DATABASE_URL; all data is in Supabase and R2.)
...
# Frontend
cd client
yarn start                      # Start dev server (uses .env.local for API URL)
yarn start:local               # Start dev server, always use local backend (http://localhost:5001)
yarn test                      # Run tests
...
```
(No SQLAlchemy; all metadata is in Supabase.)
...
| `SUPABASE_SERVICE_ROLE_KEY` | `eyJ...` | Yes | Supabase service role key |
| `SUPABASE_ANON_KEY` | `eyJ...` | Yes | Supabase anon key (auth) |
...
| `REACT_APP_API_BASE_URL` | `http://localhost:5001` or prod URL | Yes (client) | Backend API base URL |
