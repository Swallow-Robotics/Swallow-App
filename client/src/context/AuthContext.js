...
export const AuthProvider = ({ children }) => {
  ...
  const sessionRef = useRef(authState.session);
  ...
  }, [refreshProjects, isRetryableError]);

  // Ref so the session effect doesn't re-run when loadProjectsWithRetry identity changes
  // (it changes when authState updates after projects fetch, which would otherwise loop).
  const loadProjectsWithRetryRef = useRef(loadProjectsWithRetry);
  loadProjectsWithRetryRef.current = loadProjectsWithRetry;
  ...
  useEffect(() => {
    ...
    const restoreSession = async () => {
      try {
        ...
        if (error) {
          ...
          if (data?.session) {
            loadProjectsWithRetryRef.current();
          }
        }
      } catch (err) {
        if (isMounted) {
    ...
    const {
      ...
      if (nextSession) {
        // Avoid refetching projects on every token refresh; only on sign-in or initial session.
