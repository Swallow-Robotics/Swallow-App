...
# Check if yarn is installed (frontend uses yarn)
if ! command -v yarn &> /dev/null; then
    print_error "yarn is not installed. Install it with: npm install -g yarn"
    exit 1
fi
...
echo "   ${BLUE}Frontend:${NC} cd client && yarn start"
echo ""
...
echo "   ${BLUE}Frontend:${NC} cd client && yarn test"
echo ""
echo "📚 Documentation:"
